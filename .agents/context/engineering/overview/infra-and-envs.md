...
- **Vercel** (`.github/workflows/deploy-vercel.yml`) — `apps/web/vercel.json` sets `git.deploymentEnabled: false`, disabling Vercel's own Git-triggered deploys (production **and** PR preview deployments) for every branch, in-repo and versioned rather than a dashboard toggle. This deliberately reintroduces a `vercel.json` after an earlier commit removed one — that removal was about a now-unneeded `installCommand`/`buildCommand` override (Vercel's native Turborepo detection covers that); `git.deploymentEnabled` is unrelated and doesn't reintroduce that problem. The Git repository connection itself stays intact (Root Directory, framework preset, env var sync all still depend on it) — only automatic deployment-on-push is turned off.
  ...
  Deploys go through the [`BetaHuhn/deploy-to-vercel-action`](https://github.com/marketplace/actions/deploy-to-vercel-action) (`@v1`) instead of a hand-rolled `vercel pull`/`build`/`deploy` CLI sequence — it also creates a GitHub Deployment and a PR comment with the preview URL, restoring the preview-on-PR behavior that `git.deploymentEnabled: false` otherwise removes, but still fully CI-driven rather than Vercel's own git integration. Triggered on `push` to **both** `dev` and `main`, plus `pull_request` targeting `dev`; the action's `PRODUCTION` input is left unset so it defaults to a production deployment on push and a preview deployment on PRs. `main` is included ahead of the rest of this repo's convention (every other CI workflow is `dev`-only until `main` exists as a real branch, per "Environment structure" below) — this is a deliberate exception for the Vercel job specifically, not yet mirrored on the Heroku side or the other workflows. **Open consideration:** once `main` exists for real, both `dev` and `main` pushing to the same Vercel Production alias/domain would race — this needs revisiting (e.g. a real prod Doppler config + separate Vercel env) before `main` becomes a real branch, not left as-is.

- **Heroku** (`.github/workflows/deploy-heroku.yml`) — app → Deploy tab → if a GitHub auto-deploy branch is configured, disable it by hand (Heroku has no in-repo equivalent to `vercel.json`'s `git.deploymentEnabled`; and don't use `git push heroku` / `heroku.yml`-based deploys either — those are also git-triggered). The `deploy-api` job builds a Docker image from `apps/api/Dockerfile` (build context = repo root, per the Yarn workspaces dependency chain — see the Dockerfile's own comments), pushes it to Heroku's Container Registry, and releases it via the Platform API's `formation` endpoint (`Accept: application/vnd.heroku+json; version=3.docker-releases`) — the documented imperative Docker-deploy flow, no Heroku CLI install needed in CI. Still `dev`-only, unlike Vercel's job.

Both workflows fetch their credentials from Doppler's `batuto-deploy` project (see above). `deploy-heroku.yml`'s job runs under a GitHub Environment named `dev` (Settings → Environments) for deployment visibility/protection-rules; `deploy-vercel.yml` deliberately doesn't set one, since a single job now serves three different triggers (`dev` push, `main` push, PR preview) that don't map cleanly onto one environment name — revisit once real `dev`/`prod` Vercel environments exist. Neither workflow duplicates secrets by hand outside Doppler; `GITHUB_TOKEN` in `deploy-vercel.yml` is the default per-workflow token GitHub Actions injects automatically, not a Doppler-managed secret — it only needs `deployments: write`/`pull-requests: write` permissions (set at the workflow level) to create the GitHub Deployment and PR comment.
