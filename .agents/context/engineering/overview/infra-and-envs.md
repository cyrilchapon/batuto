...
1. **Build (parallel)** — `build-api` (`docker build`/`docker push` `apps/api/Dockerfile` to `registry.heroku.com/$HEROKU_APP_NAME/web`) and `build-web` (`vercel pull --environment=<production|preview>` — fetches the project's linked settings **and** its configured Environment Variables, still how Doppler's Vercel integration reaches the build, just consumed via `vercel pull` instead of Vercel's own remote build reading it directly → `vercel build [--prod]`, Turborepo-aware the same way Vercel's removed remote `buildCommand` override used to be — see below — and, thanks to `vercelPreset()`, produces a real Build Output API `.vercel/output/` directory → uploads it as a build artifact, since separate jobs don't share a filesystem) run with no dependency on each other.
