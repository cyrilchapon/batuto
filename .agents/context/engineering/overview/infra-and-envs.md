...
3. **Deploy (parallel)** — `deploy-api` and `deploy-web` both `needs: release`, neither needs the other. `deploy-api`: `heroku container:login` → `docker pull` the image `build-api` pushed (this job runs on a fresh runner, so the image isn't there yet — `container:release` inspects images via the local Docker daemon, not the registry directly) → `heroku container:release web --app $HEROKU_APP_NAME`, which promotes the pulled image to a new Heroku Release (still gives real Heroku release history/`heroku rollback`, same as any other Heroku deploy — rollback never un-runs a migration either way, with or without Heroku's own release-phase coupling, so nothing is lost by keeping migrations outside it). `deploy-web`: downloads `build-web`'s artifact, deploys via [`BetaHuhn/deploy-to-vercel-action`](https://github.com/marketplace/actions/deploy-to-vercel-action) (`@v1`) with `PREBUILT: true` (only appends `--prebuilt` to its `vercel deploy` call, uploading the already-built output — Vercel never sees or builds from source) and `PRODUCTION` following the `vercel-prod` input.
