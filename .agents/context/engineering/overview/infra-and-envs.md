...
`.github/workflows/deploy-production.yml` (`push: main`) and `deploy-staging.yml` (`push: dev`) are thin triggers — a few lines each — that call `.github/workflows/_reusable-deploy.yml` (`on: workflow_call`, `secrets: inherit`) with `gh-environment: Production|Staging` and `vercel-prod: true|false`. The two tiers are genuinely the same pipeline, only their targets differ, so the pipeline itself is written once.

Job graph inside `_reusable-deploy.yml` — **CLIs (Heroku CLI, Vercel CLI) over raw platform APIs wherever one exists**, both for reliability and because the CLIs handle things a hand-rolled `curl` sequence would otherwise have to reimplement (image ID lookup, release polling/streaming):

1. **`release-api`** — build, push, *and* release, all in one job. `docker build --target web` and `--target release` from `apps/api/Dockerfile` (see "Dockerfile stages" below) → `docker push` both to `registry.heroku.com/$HEROKU_APP_NAME/{web,release}` → `heroku container:release web release --app $HEROKU_APP_NAME`. This is the actual Heroku release-phase mechanism: Heroku runs `release` (the Dockerfile's `release` stage, `migrate:deploy`) as a one-off dyno first, and only promotes `web` to the new release if that exits 0 — gated atomically by Heroku itself, not a CI script's exit code, so `heroku releases`/`heroku rollback` work against the result same as any other Heroku deploy. `container:release` blocks until release phase finishes and fails the step on a bad migration. **This is why Heroku's "release" and "deploy" aren't two separate CI-controlled steps the way Vercel's build/deploy are** — they're one atomic release object on Heroku's side, and why build+push+release stay in a single job: `container:release` needs the just-pushed images in the same Docker daemon it inspects, so splitting build/push from release across separate runners doesn't work.
2. **`build-web`** — runs in the GitHub Actions runner, not on Vercel's remote build infrastructure: `vercel pull --environment=<production|preview>` (fetches the project's linked settings **and** its configured Environment Variables — this is still how Doppler's Vercel integration reaches the build, just consumed via `vercel pull` instead of Vercel's own remote build reading it directly) → `vercel build [--prod]` (Turborepo-aware the same way Vercel's removed remote `buildCommand` override used to be — see below — and, thanks to `vercelPreset()`, produces a real Build Output API `.vercel/output/` directory) → uploads that directory as a build artifact (separate jobs don't share a filesystem).
3. **`deploy-web`** (`needs: [release-api, build-web]`) — downloads the artifact from step 2, deploys via [`BetaHuhn/deploy-to-vercel-action`](https://github.com/marketplace/actions/deploy-to-vercel-action) (`@v1`) with `PREBUILT: true` (only appends `--prebuilt` to its `vercel deploy` call, uploading the already-built output — Vercel never sees or builds from source) and `PRODUCTION` following the `vercel-prod` input. Gating this on `release-api` is what makes "deploy only after release success" real; it's its own job (not nested inside `release-api`) so it doesn't block on Heroku's *web promotion* specifically, only on the shared release-success gate — as parallel as two platforms with different release/deploy couplings can practically be.

Every job that reads `vars.HEROKU_APP_NAME`/`vars.VERCEL_ORG_ID`/`vars.VERCEL_PROJECT_ID` sets `environment: ${{ inputs.gh-environment }}` so those resolve from the right GitHub Environment; `secrets.HEROKU_API_KEY`/`secrets.VERCEL_TOKEN` are repository secrets, unaffected by environment scoping. `GITHUB_TOKEN` is the default per-workflow token GitHub Actions injects automatically, not a manually-set secret — the workflow's `permissions: deployments: write, pull-requests: write` is what lets the Vercel action create a GitHub Deployment and PR comment. The Heroku CLI itself isn't preinstalled on `ubuntu-latest` runners — every job that needs it runs `curl https://cli-assets.heroku.com/install.sh | sh` first; it authenticates non-interactively from `HEROKU_API_KEY` in the environment, same as `heroku container:login`.

#### PR review apps — `deploy-preview.yml`

Any PR targeting `dev` gets an ephemeral, fully-linked review stack, replicating the staging/production pipeline rather than being a simplified stand-in for it:

1. **`neon-branch`** — creates or reuses a Neon branch named `pr-<number>`, forked from the shared dev branch (same `create-branch-action` pattern as `db-checks.yml`'s `check:migrations` job — a real, data-bearing clone), but with **no `expires_at`**: unlike `db-checks.yml`'s per-run ephemeral branches, this one lives for the PR's whole lifetime, deleted only by the `teardown` job below. `create-branch-action` is itself idempotent on `branch_name` (confirmed via its own docs: an existing branch with that name is reused, not duplicated or errored on), so re-pushing to the same PR (`synchronize`) doesn't create a second branch.
2. **`deploy-api-review`** (`needs: neon-branch`) — one job, same reasoning as `release-api` above: computes a deterministic app name (`batuto-api-pr-<number>`, so `https://batuto-api-pr-<number>.herokuapp.com` needs no lookup), `heroku apps:info || heroku apps:create` (Container Registry pushes require the app to exist first), `heroku config:set` from `batuto-api`'s Doppler `dev` config (same source `build-and-test.yml` already fetches) **except** `DATABASE_URL` (step 1's Neon clone, never the shared dev database) — then the same build/push/`heroku container:release web release` as `release-api`.
3. **`deploy-web-review`** (`needs: deploy-api-review`) — same `vercel pull --environment=preview` → `vercel build` → prebuilt deploy flow as staging/production, with one addition: after `vercel pull`, strips any existing `VITE_API_URL=` line from `.vercel/.env.preview.local` and appends `VITE_API_URL=<step 2's API URL>` (strip-then-append rather than relying on "last line wins" in a dotenv file, which isn't a guaranteed parsing rule) — **this is the actual web↔api review-app link.** Runs under the existing `Preview` GitHub Environment for `vars.VERCEL_ORG_ID`/`vars.VERCEL_PROJECT_ID`.
4. **`link-cors`** (`needs: [deploy-api-review, deploy-web-review]`) — closes the loop the other direction. The API review app boots in step 2 with `WEB_URL` unset (it has a harmless default, `http://localhost:5173` — see `apps/api/src/env.ts` — so this doesn't crash anything, just means CORS rejects the real preview origin until now); once the web preview's real URL is known (`deploy-to-vercel-action`'s `PREVIEW_URL` output), `heroku config:set WEB_URL=...` patches it onto the API app. Heroku restarts the dyno on config var change — no redeploy needed.
...
`apps/api/Dockerfile`'s `build` stage (turbo build of api's full dependency chain — see the Dockerfile's own comments) feeds two named final stages that share every cached layer, making the second nearly free to build after the first:
...
CMD ["yarn", "workspace", "@batuto/db", "migrate:deploy"]
```

`--target web`/`--target release` at build time is what lets every tier above push both images and let Heroku's own release-phase mechanism gate promotion on migrations succeeding.
